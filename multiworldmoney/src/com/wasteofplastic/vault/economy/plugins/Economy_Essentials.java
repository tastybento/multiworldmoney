 ...
 */
package com.wasteofplastic.vault.economy.plugins;
...
import com.wasteofplastic.vault.economy.EconomyResponse.ResponseType;
...
public class Economy_Essentials implements Economy {
    ...
    public Economy_Essentials(Plugin plugin) {
        ...
        if (ess == null) {
        	...
        	log.info(String.format("[%s][Economy] %s not hooked.", plugin.getDescription().getName(), name));
            }
        }
    }
    ...
    @Override
    public EconomyResponse setBalance(String playerName, double amount) {
        double balance;
        EconomyResponse.ResponseType type;
        String errorMessage = null;
        try {
            com.earth2me.essentials.api.Economy.setMoney(playerName,amount);
            balance = com.earth2me.essentials.api.Economy.getMoney(playerName);
            type = EconomyResponse.ResponseType.SUCCESS;
        } catch (UserDoesNotExistException e) {
            if (createPlayerAccount(playerName)) {
    ...
    @Override
    public EconomyResponse setBalance(String playerName, double amount, String world) {
	return setBalance(playerName, amount);
    }
}